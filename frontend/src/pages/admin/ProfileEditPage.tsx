...
import { Save, X, Loader2, ChevronRight, FileText, Download, Trash2 } from 'lucide-react'
...
export const ProfileEditPage = () => {
  ...
  const handleAvatarDrop = async (file: File) => {
  ...
  const handleResumeDrop = async (file: File) => {
    if (file.type !== 'application/pdf') {
      toast.error('请上传 PDF 格式的简历')
      return
    }
            ...
            <div className="flex items-center gap-[var(--space-lg)]">
              <div
                ...
                onDrop={(e) => {
                  ...
                  if (file) handleAvatarDrop(file)
                }}
                onClick={() => avatarInputRef.current?.click()}
                className={`w-20 h-20 rounded-[var(--radius-full)] overflow-hidden border-2 flex-shrink-0 cursor-pointer transition-all duration-[var(--duration-base)]
                    ${
                      avatarDragOver
              ...
              >
                {avatarUrl ? (
                  <img src={avatarUrl} alt="当前头像" className="w-full h-full object-cover" />
                ) : (
              ...
              <div>
                <div
                  ...
                  onClick={() => avatarInputRef.current?.click()}
                  className={`border-2 border-dashed rounded-[var(--radius-md)] px-[var(--space-md)] py-3 cursor-pointer transition-colors text-center
                      ${
                        avatarDragOver
                ...
                >
                  <p className="text-sm text-[var(--color-secondary)]">
                    {uploadAvatarMutation.isPending ? '上传中...' : '拖拽图片到此处或点击上传'}
                  </p>
                </div>
                <input
                  ref={avatarInputRef}
          ...
          {resume ? (
            ...
            >
              ...
              {uploadResumeMutation.isPending && (
                <Loader2 className="w-5 h-5 animate-spin mx-auto text-[var(--color-accent)]" />
              )}
              <input
                ref={resumeInputRef}
                type="file"
          ...
          >
            ...
            >
              <div className="flex flex-col items-center text-center">
                ...
                <h2 className="text-lg font-semibold text-[var(--color-primary)] mb-2">确认删除</h2>
